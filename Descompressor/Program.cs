...
namespace StreamTest
{
    class Program
    {
        ...
        {
            // Creation of varrable of type string
            string line;

            // Create an instance of Filestream getting the file that was
            // created and open it and read it
            FileStream fs = 
                new FileStream(ficheiro, FileMode.Open, FileAccess.Read);

            // Get file type and decompress it
            GZipStream gzs = new GZipStream(fs, CompressionMode.Decompress);

            // Instance of type streamreader to get acess to StreamReader
            // properties
            StreamReader sr = new StreamReader(gzs);

            // Write all string till the end of file on the screen
